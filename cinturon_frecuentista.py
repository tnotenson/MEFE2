...
ax[1].grid()
plt.savefig(f't_vs_tau_n{n}_CL{CL}_Nexp{Nexp}_fun{sfun}.png', dpi=200)
#%%
...
def IsInside2(nobs,mu, CI=False):
    ...
    dom = np.linspace(0.001, mu+10*np.sqrt(mu),10000)
    img = LLvec(nobs,dom); y = max(img) - 1/2
    arr1 = np.where(img >= y)
    ...
    for i in range(0,nscan_points):
        ...
        for nobs in range(0,Nmax):
            ...
            if IsInside == IsInside2:
                inside2 = IsInside(nobs,mu)
            elif IsInside == IsInside_CI:
                inside2 = IsInside(nobs,mu,funt)
                
            prob = stats.poisson.pmf(nobs,mu);
