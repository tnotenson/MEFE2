...
for i in range(0,nscan_points):
    ...
    for iToy in range(0,nToys):
        efi=0.0
   	# Tenemos que pensar bien cual es la distribucion del posterior para efi y para b
        # get Efficiency from truncated Gaussian
        while (efi<=0): efi=rnd.Beta(eObs+1,nObsEff/nTotalEff) #rnd.Gaus(eObs,eErr*eObs); # Sigma for Binomial is above

        b=0.0;
        # get Background from truncated Gaussian 
        while (b<=0): b=rnd.Poisson(bObs,np.sqrt(bObs)) #rnd.Gaus(bObs,bObs*bErr); # Sigma for Poisson is sqrt(mean)
...
nombre = input('Presione alguna tecla para terminar')
